...
namespace dynamixel_wrapper {
	Wrapper::Wrapper() {
		...
		speed_writer = std::make_unique<dynamixel::GroupSyncWrite>(port.get(), packet.get(),
				MX106_PROFILE_VELOCITY_ADDR, MX106_PROFILE_VELOCITY_SIZE);
	...
	ComError Wrapper::enable(bool enable) {
		const uint8_t on_off = static_cast<uint8_t>(enable);
		return static_cast<ComError>(packet->write1ByteTxOnly(port.get(),
					BROADCAST_ID, MX106_TORQUE_ENABLE_ADDR, on_off));
	}

	ComError Wrapper::enable(uint8_t id, bool enable) {
		const uint8_t on_off = static_cast<uint8_t>(enable);
		return static_cast<ComError>(packet->write1ByteTxOnly(port.get(),
	...
	ComError Wrapper::read_state(std::vector<ServoState>& out) {
		...
		for(const auto& id : DYRET_SERVO_IDS) {
			...
			st.velocity_trajectory = state_reader->getData(id, MX106_VELOCITY_TRAJECTORY_ADDR, MX106_VELOCITY_TRAJECTORY_SIZE);
			st.position_trajectory = state_reader->getData(id, MX106_POSITION_TRAJECTORY_ADDR, MX106_POSITION_TRAJECTORY_SIZE);
