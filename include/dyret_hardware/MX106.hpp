...
/**
 * This file contains constants related to the Dynamixel MX-106 (2.0) servo
 */
...
namespace mx106 {
	float voltage_from_raw(const uint16_t volt) {
		return static_cast<float>(volt) / 10.0;
	}

	float velocity_from_raw(const uint32_t vel) {
		float val = static_cast<float>(static_cast<int32_t>(vel));
		// Convert to RPM:
		val *= 0.229;
		// Convert to Rad/sec
		val *= 0.10472;
	...
	float current_from_raw(const uint16_t current) {
		float val = static_cast<float>(static_cast<int16_t>(current));
		// Convert to mA:
		val *= 3.36;
		return val;
	}

	float temp_from_raw(const uint8_t temp) {
		return static_cast<float>(temp);
	}

	float pwm_from_raw(const uint16_t pwm) {
		float val = static_cast<float>(static_cast<int16_t>(pwm));
		val /= 885;
		return val;
	}
}
